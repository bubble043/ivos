...
// 定义数组用来保存面包屑导航显示的值（增加错误处理）
const breadcrumb = ref([]);

try {
  const raw = localStorage.getItem("breadCrumb");
  breadcrumb.value = raw && raw !== "undefined"
      ? JSON.parse(raw)
      : ["用户管理", "用户列表"];
} catch (e) {
  console.warn("无效的 breadCrumb 数据，使用默认值");
  breadcrumb.value = ["用户管理", "用户列表"];
}

// 维护面包屑导航与菜单项 index 的对应关系
let map = {
  "/user": ["用户管理", "用户列表"],
  "/vehicle": ["车辆管理", "基本信息"],
  "/geofence": ["电子围栏", "围栏管理"],
...
const selectMenu = (index) => {
  breadcrumb.value = map[index] || ["未知", "未知"];
  // 解决面包屑刷新丢失问题: 存到 localStorage 中
